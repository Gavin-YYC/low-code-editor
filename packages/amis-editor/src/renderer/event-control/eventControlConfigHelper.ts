...
export const actionConfigInitFormatterHoc =
  ...
  async (action: ActionConfig) => {
    ...
    if (action.actionType === 'toolStream') {
      // action.api = {
      //   url: 'http://localhost:3000/stream-output',
      //   method: 'get',
      //   requestAdaptor: '',
      //   adaptor: '',
      //   messages: {},
      //   dataType: 'json',
      //   data: {id: action.toolSelect.id, ...action.queryParam},
      //   responseType: 'blob'
      // };
      config.api = action.api;
      // config.options =
      // TODO xuzg 访问工作流工具，传入参数{id: action.toolSelect.id, ...action.queryParam}，
      // 仿照ajax的处理，构造config.api和config.options，
      console.log('action.args', action, action.queryParam);
    }
