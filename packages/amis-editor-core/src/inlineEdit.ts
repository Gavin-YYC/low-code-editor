...
import type {InlineEditableElement} from './plugin';
...
function startPlainTextEdit({
  ...
  const origin = elem.innerText.trim();
  const forceCancel = false;
...
function getMouseEventCaretRange(evt: MouseEvent, elem: HTMLElement) {
  ...
  if ((doc.body as any).createTextRange) {
    ...
    // Try the standards-based way next
      ...
      const pos: any = (doc as any).caretPositionFromPoint(x, y);
      range = doc.createRange();
...
function selectRange(range: any, elem: HTMLElement) {
  ...
  if (range) {
    if (typeof range.select != 'undefined') {
      ...
      const sel: any = win.getSelection();
      sel.removeAllRanges();
      sel.addRange(range);
    }
  }
