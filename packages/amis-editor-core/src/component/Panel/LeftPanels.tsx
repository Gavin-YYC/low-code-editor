import {Icon, Tab, Tabs} from 'amis';
...
> {
  ...
  // #YueZhan：需要隐藏左侧面板：全局变量和代码展示
  // 这里做显隐简单，在store里面修改数据需要改大量的类型
  // hideKey = ['code','globalVar'];
  hideKey = ['global-var', 'code'];

  render() {
    const {store, manager, theme} = this.props;
    const {isFixedStatus} = this.state;
    const leftPanelOpenStatus = store.leftPanelOpenStatus;
    // #YueZhan：负责渲染最左侧的图标面板
    const panels = store
      .getLeftPanels()
      .filter(item => !this.hideKey.includes(item.key));

    // console.log(panels, '最终渲染的面板');
