...
import {EditorStoreType, MainStore} from '../store/editor';
import type {EditorNodeType} from '../store/node';
import {autobind, reGenerateID} from '../util';
import type {VariableGroup, VariableOptions} from '../variable';
import Breadcrumb from './Breadcrumb';
import {ModalForm} from './ModalForm';
import {ContextMenuPanel} from './Panel/ContextMenuPanel';
import {LeftPanels, LeftPanelsProps} from './Panel/LeftPanels';
import {RightPanels, RightPanelsProps} from './Panel/RightPanels';
...
export default class Editor extends Component<EditorProps> {
  ...
  componentDidUpdate(prevProps: EditorProps) {
    ...
    if (
      props.value.title !== prevProps.value.title &&
      props.value.id === prevProps.value.id
    ) {
      const {id, title} = props.value;
      if (window?.amisData.editting_schema.id === id) {
        window.amisData.editting_schema.title = title;
      }
      const pageOptionItem = window?.amisData.page_options.find(
        item => item.value === id
      );
      if (pageOptionItem) {
        pageOptionItem.label = title;
      }
      const schemaCacheItem = window?.amisData.schema_cache_list.find(
        item => item.id === id
      );
      if (schemaCacheItem) {
        schemaCacheItem.title = title;
      }
    }
  ...
  render() {
    ...
    // #YueZhan:左侧面板，这块没有传入自定义的面板，用的默认面板
