...
export interface FileControlSchema extends FormBaseControlSchema {
  ...
  /**
   * 默认 `/console/api/files/upload` 如果想自己存储，请设置此选项。
   *
   * @default /console/api/files/upload
   */
  receiver?: SchemaApi;
...
export default class FileControl extends React.Component<FileProps, FileState> {
  ...
  async _send(
    ...
    return this._customSend(file, api, options, onProgress, maxRetryLimit);
  }
  ...
  async _customSend(
    ...
    try {
      ...
      const {token = ''} = amisEditorProps || {};
      const upload = () => {
        return new Promise((resolve, reject) => {
          const xhr = new XMLHttpRequest();
          xhr.open('post', typeof api === 'string' ? api : api.url);
          xhr.setRequestHeader('Authorization', token);
          xhr.setRequestHeader('operToken', token);
