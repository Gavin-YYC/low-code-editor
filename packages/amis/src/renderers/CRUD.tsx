...
export default class CRUD<T extends CRUDProps> extends React.Component<T, any> {
  ...
  pickColumnsData() {
    ...
    const maxRows = columns.map((col: any) => {
      if (col.columnsData) {
        return col.columnsData.length;
      }
      return 0;
    });
    // 获取最大数据行数
    const max = Math.max(...maxRows);
    const result = [];
    for (let i = 0; i < max; i++) {
      const obj = {};
      columns.forEach((col: any) => {
        if (col.columnsData) {
          obj[col.name] = col.columnsData[i]?.value;
        }
      });
      result.push(obj);
    }
