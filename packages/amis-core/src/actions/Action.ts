...
export const runAction = async (
  ...
  if (action.actionType === 'dialog') {
  ...
  } else if (action.actionType === 'toolStream') {
    const {props = {}} = window.$wujie || {};
    const {amisEditorProps = {}} = props;
    const {url, appId = '', token = ''} = amisEditorProps || {};
    let reqUrl = `${url}/console/api/workspaces/tool-run`;
    let reqData = {};
    // 工作流工具先通过toolId获取workflowId
    if (action.toolSelect.type === 'workflow') {
      const response = await fetch(
        `${url}/console/api/workspaces/current/tool-provider/workflow/get?workflow_tool_id=${action.toolSelect.id}`,
        {
          method: 'get',
          headers: {
            'Authorization': token,
      ...
      reqUrl = `${url}/console/api/apps/${resBody.workflow_app_id}/workflows/draft/run`;
      reqData = {
        files: [],
        inputs: action.queryParam
      };
    } else {
      reqData = {
        parameters: action.queryParam
      };
    }
    ...
    action.api = {
      ...
      messages: {},
      dataType: 'json',
      // id为工具的唯一id，工具调用的事件发布和监听也用这个字段
      data: {
        tool_name: action.toolSelect.name,
        tool_provider_id: action.toolSelect.id,
        tool_provider_type: action.toolSelect.type,
        ...reqData
      },
      responseType: 'stream'
    };
  ...
  const targetComponent = cmptFlag
  ...
  const data =
    ...
    !['ajax', 'download', 'toolStream', 'dialog', 'drawer'].includes(
      action.actionType
  ...
  console.debug(`[${action.actionType}] action args, data`, args, data, action);
